...
4. **本文目标** (1) 如何将 2D 运动分割的预测有效融合到动态 NeRF 的分层表示中？(2) 如何克服长视频中场景过于复杂导致几何建模不足的问题？
...
6. **核心 idea**：通过正向运动融合（PMF）和负向运动融合（NMF）将 2D 运动分割蒸馏到分层 NeRF 的动态和半静态层中，并用测试时精修聚焦特定帧以缓解几何建模不足的问题。
