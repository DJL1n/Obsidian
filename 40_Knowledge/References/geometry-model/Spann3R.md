...
4. **不输出显式 pose-depth optimization trace** — 无 [[slam-frontends/patch-based/DPVO]]/DROID 式 BA residual/confidence/lifecycle
...
### 合理位置
Spann3R output → CandidateGeometryPacket → cross-check with [[slam-frontends/patch-based/DPVO]] + depth-normal + free-space → global correction → CertifiedGeometryPacket → GS
...
- [[slam-frontends/gpu-optimized/GO-SLAM]]-style correction (global BA)
- [[slam-frontends/patch-based/DPVO]]-style temporal evidence (lifecycle)
- [[gs-slam/monocular/GS-SLAM]]-style alpha feedback (gating)
...
|---|---|---|
| [[slam-frontends/patch-based/DPVO]] | temporal tracking | temporal backbone |
...
| [[slam-frontends/gpu-optimized/GO-SLAM]] | online global LC/BA | global correction |
| [[gs-slam/structured/Scaffold-GS]] | structured GS backend | anchor-conditioned ChildGS |
...
### Concepts
- [[Spatial-Memory-Reconstruction]] — external spatial memory for global-coordinate pointmap
- [[Dense-Sparse-Memory]] — working memory + long-term memory architecture
...
- **前作**：[[geometry-priors/feed-forward/DUSt3R]], [[geometry-priors/feed-forward/MASt3R]]（DUSt3R 家族引入空间记忆）
- **后继**：[[geometry-priors/feed-forward/CUT3R]]
