...
OpenMonoGS-SLAM = [[geometry-priors/feed-forward/MASt3R]]-SLAM geometry backbone + SAM multi-scale masks + CLIP language features + 3DGS semantic feature rendering + memory bank。只用 monocular RGB，构建可渲染、可开放词汇查询的 3D Gaussian semantic map。不是动态 SLAM / 不解决 pose-depth-scale coherence / 但 memory bank + multi-scale SAM + CLIP 的语义 map 架构很有参考价值。

---

## 1. 解决什么问题

**几何缺口**：monocular GS-SLAM 的几何精度和稳定性仍弱于 RGB-D 系统 → 用 [[geometry-priors/feed-forward/MASt3R]]-SLAM 当 backbone。
...
```
Monocular RGB → [[geometry-priors/feed-forward/MASt3R]]-SLAM tracking (pointmap + ray-error)
→ [[geometry-priors/feed-forward/MASt3R]] pointmap → 3DGS init (pos/rot/scale/opacity/RGB + learnable semantic feature)
→ SAM multi-scale masks → lift to 3D via [[geometry-priors/feed-forward/MASt3R]] depth → scale-conditioned membership
...
### [[geometry-priors/feed-forward/MASt3R]]-SLAM backbone
Tracking + pointmap estimation 来自 [[geometry-priors/feed-forward/MASt3R]]-SLAM（ray-error optimization + confidence-weighted robust pose）。几何不是论文创新的重点。
...
1. Lift SAM masks 到 3D via [[geometry-priors/feed-forward/MASt3R]] depth + intrinsics
...
(ATE 提升主要来自 [[geometry-priors/feed-forward/MASt3R]]-SLAM backbone)
...
1. **几何前端依赖 [[geometry-priors/feed-forward/MASt3R]]-SLAM** — 不解决 pose-depth-scale coherence 根本问题
2. **VFM pipeline 重** — [[geometry-priors/feed-forward/MASt3R]] + SAM + CLIP + Grounded-SAM
3. **动态场景鲁棒性不足** — 论文自己承认
...
Coarse: 大面/大物体稳定支持。Mid: 边界/法线过渡。Fine: 高频细节 GS birth。与 [[mapping/semantic/RGBDS-SLAM]] pyramid + [[gs-slam/monocular/RTG-SLAM]] lifecycle 合并。
...
### 不建议借
- [[geometry-priors/feed-forward/MASt3R]]-SLAM 几何前端直接作为你的几何认证 authority
...
### Methods
- [[OpenMonoGS-SLAM-Architecture]] — [[geometry-priors/feed-forward/MASt3R]] + SAM + CLIP + memory bank
