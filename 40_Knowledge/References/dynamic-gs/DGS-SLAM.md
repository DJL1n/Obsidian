...
普通 GS-SLAM ([[gs-slam/rgbd/SplaTAM]]/[[gs-slam/monocular/MonoGS]]) 默认静态场景，动态物体会：
...
First frame: depth unprojection, exclude dynamic regions, fill depth holes。New keyframe: insert Gaussians 时排除动态区域和 invalid depth。沿用 3DGS/[[gs-slam/monocular/MonoGS]] densification/pruning。
...
|---|---|---|---|
| [[gs-slam/rgbd/SplaTAM]] | 15.55 | 0.633 | 0.413 |
| [[gs-slam/monocular/MonoGS]] | 15.89 | 0.621 | 0.358 |
| **DGS-SLAM** | **20.63** | **0.807** | **0.186** |

### Tracking (TUM dynamic)
| Method | Avg ATE |
|---|---|
| [[gs-slam/rgbd/SplaTAM]] | 166.0 cm |
| [[gs-slam/monocular/MonoGS]] | 37.7 cm |
| **DGS-SLAM** | **3.0 cm** |
...
|---|---|
| ORB-SLAM3 | 29.8 cm |
| [[gs-slam/monocular/MonoGS]] | 32.9 cm |
| **DGS-SLAM** | **7.3 cm** |
| DynaSLAM | 4.8 cm |
...
### Runtime
~1.6 FPS (不含 segmentation)，[[gs-slam/monocular/MonoGS]] ~1.07 FPS。
...
Segmentation 不是最终结论 → 必须被几何一致性验证。你的版本：[[slam-frontends/patch-based/DPVO]]/DROID/HI-SLAM2 prediction + [[geometry-priors/feed-forward/MASt3R]] consistency + read-only anchor support，不是 GS render。

### 不建议借：GS residual 直接影响前端
DGS-SLAM 的核心闭环（GS render → residual mask → tracking/mapping）在你的 pre-certification 阶段危险。你的边界：frontend authority = [[slam-frontends/patch-based/DPVO]]/DROID/anchor；backend evaluator = GS rendering only evaluates, 不反向改 pose/depth/anchor。
...
### Concepts
- [[Dynamic-Gate-Through-Pipeline]] — 动态过滤贯穿 tracking + mapping
- [[Primitive-Birth-Provenance]] — source keyframe ID for GS primitive
...
- **后继**：[[gs-slam/dynamic/ADD-SLAM]], [[gs-slam/dynamic/Dy3DGS-SLAM]], [[gs-slam/dynamic/CAD-SLAM]], [[gs-slam/dynamic/WildGS-SLAM]]
