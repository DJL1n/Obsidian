...
ContextGS 是一个 3D Gaussian Splatting 压缩方法。构建在 [[gs-slam/structured/Scaffold-GS]] 上，在 anchor level 做自回归上下文建模：先编码 coarse anchors，再用已编码的 coarse anchors 预测 finer anchors 的概率分布，然后做 entropy coding。不是 SLAM / tracking / geometry certification / online mapping。

定位：[[gs-slam/structured/Scaffold-GS]] 表示上的压缩层。

---

## 1. 核心问题

3DGS 高质量实时渲染，但模型巨大（BungeeNeRF 平均 ~1.6 GB）。[[gs-slam/structured/Scaffold-GS]] 通过 anchor 减少 Gaussian-level 冗余，但 anchor 之间仍有空间冗余。ContextGS：如何进一步压缩 anchor-level redundancy？

方案：hierarchical anchor levels + autoregressive context model + hyperprior feature + entropy coding。

---

## 2. 与 [[gs-slam/structured/Scaffold-GS]] 关系

[[gs-slam/structured/Scaffold-GS]]: anchor → Gaussian（解决 Gaussian-level 冗余）
ContextGS: anchors 编码压缩（解决 anchor-level 冗余）

[[gs-slam/structured/Scaffold-GS]] 是结构化渲染表示；ContextGS 是该表示的压缩编码器。解码后渲染与 [[gs-slam/structured/Scaffold-GS]] 一致，无额外开销。
...
Bitstream → decode hyperprior → decode L0 anchors → context → decode L1 → context → decode L2 → full anchor features → [[gs-slam/structured/Scaffold-GS]] render。渲染零额外开销。
...
### 压缩比
| Dataset | 3DGS | [[gs-slam/structured/Scaffold-GS]] | ContextGS low | ContextGS high |
|---|---|---|---|---|
...
### 消融 (BungeeNeRF)
- [[gs-slam/structured/Scaffold-GS]]: 183.0 MB, 26.62 PSNR
...
2. **压缩比极高** — >100× vs 3DGS, ~15× vs [[gs-slam/structured/Scaffold-GS]]
3. **渲染零额外负担** — 解码后与 [[gs-slam/structured/Scaffold-GS]] 相同
...
2. **依赖 [[gs-slam/structured/Scaffold-GS]] anchor 表示** — 非通用
...
| [[gs-slam/structured/Scaffold-GS]] | anchor → child Gaussian representation |
| [[mapping/structured/OG-Mapping]] | octree anchor + online growth |
| [[gs-slam/monocular/MonoGS]]/[[gs-slam/rgbd/SplaTAM]]/[[gs-slam/monocular/Gaussian-SLAM]] | [[gs-slam/monocular/GS-SLAM]] tracking/mapping |
| [[slam-frontends/patch-based/DPVO]]/DROID | temporal tracking evidence |
